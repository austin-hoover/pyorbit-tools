...
class RingInjectionController:    
    def __init__(
        ...
        self.min_kicker_angles *= 1.00e-03
        self.max_kicker_angles *= 1.00e-03
        ...
        self.min_vcorrector_angle *= 1.00e-03
        self.max_vcorrector_angle *= 1.00e-03
    ...
    def set_inj_coords(self, x: float, xp: float, y: float, yp: float, **kws) -> None:
        ...
        result = opt.minimize(cost_func, guess, bounds=opt.Bounds(lb, ub), method="L-BFGS-B", options=dict(disp=2))
        angles = result.x
        return angles

    # def set_inj_coords_x(self, x: float, xp: float, **kws) -> None:

    #     _mpi_comm = orbit_mpi.mpi_comm.MPI_COMM_WORLD
    #     _mpi_rank = orbit_mpi.MPI_Comm_rank(_mpi_comm)

    #     coords = np.array([x, xp])
        
    #     def magnitude(coords: np.ndarray, scale: float = 1.00e+04):
    #         return scale * np.sum(np.square(coords))
                            
    #     def cost_func(angles_x):
    #         self.set_kicker_angles_x(angles_x)
    #         self.init_part(0.0, 0.0, 0.0, 0.0)
    #         coords_mid = self.track_part(sublattice=0)
    #         coords_end = self.track_part(sublattice=1)
    ...
    #         cost = magnitude(coords_mid[:2] - coords) + magnitude(coords_end[:2])
    #         return cost
